class TableCtrl {
  ...
  constructor (){
    ...
    this._db = new Database('https://bfranca-digital.firebaseio.com/', 'leads', {
      onAppend: data => {
  ...
  _filter (){
    ...
    this._filterData = Object.entries(this._data).reduce((obj, entry)=>{
      ...
      if(
        (item.name.toLowerCase().indexOf(this.name.toLowerCase()) > -1 || this.name == "") &&
        (item.email.toLowerCase().indexOf(this.email.toLowerCase()) > -1 || this.email == "")) {
        obj[key] = item
      };
      return obj;
    }, {});
